# SKILL 06 — SweetCoins (Sistema de Fidelización)

> **CUÁNDO USAR:** Antes de implementar el módulo `sweetcoins`, cupones, consulta de puntos, o canjes.

---

## 1. Modelo de Negocio

**SweetCoins** es el programa de puntos de fidelización de Mytrufely:
...
# Ejemplo: total=S/.50, tasa=0.10 → 5 SweetCoins
```
...
```
POST /api/v1/sweet-coins/redeem
...
async def expire_coupons_task():
    async with get_async_session() as session:
        ...
        count = result.scalar()
        logger.info("sweetcoins.coupons_expired", count=count)
    return count
```
...
## 10. Endpoints del Módulo SweetCoins

| Método | Ruta | Rol | Descripción |
|---|---|---|---|
| `GET` | `/sweet-coins/balance` | CLIENTE | Saldo actual del cliente |
| `GET` | `/sweet-coins/history` | CLIENTE | Historial de movimientos |
| `GET` | `/sweet-coins/coupons/available` | CLIENTE | Cupones maestro disponibles para canjear |
| `POST` | `/sweet-coins/coupons/redeem` | CLIENTE | Canjear cupón con puntos |
| `GET` | `/sweet-coins/coupons/mine` | CLIENTE | Cupones propios del cliente |
| `POST` | `/sweet-coins/adjust` | ADMIN | Ajuste manual de puntos |
| `GET` | `/sweet-coins/config` | ADMIN | Ver configuración activa |
| `PUT` | `/sweet-coins/config` | ADMIN | Actualizar configuración |
